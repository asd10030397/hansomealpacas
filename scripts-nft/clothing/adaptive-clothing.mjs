...
const GARMENTS = {
  "farmer-overalls": { name: "Farmer Overalls", draw: drawOveralls, hRef: "leg", note: {
    adjustments: [
      "Denim bib hugs the measured silhouette per row",
      "Shoulder straps ride to each archetype's neck sides",
      "Width follows body; wool shows at the edges",
      "Bottom clamped clear of legs/feet",
      "Sleek widened; cria pulled in; scruffy inside the wool",
    ],
    system: ["Measures neck, shoulder, chest W, torso profile", "Generates a fitted variant PNG per archetype", "Prevents clipping into wool / covering the face", "Keeps the denim workwear identity"],
    footer: "Same overalls identity. Geometry is generated per archetype so each alpaca truly wears it.",
  } },
  "farm-vest": { name: "Farm Vest", draw: drawVest, hRef: "body", note: {
    adjustments: [
      "Open-front panels follow the body silhouette",
      "Centre gap scales with body so wool reads through",
      "Folded collar seats on the shoulder line",
      "Quilt stitching + toggles scale with the panels",
      "Sleek widened; bramble/elder fill without stretching",
    ],
    system: ["Measures shoulder line, chest W, torso profile", "Generates a fitted variant PNG per archetype", "Prevents clipping into wool / covering the face", "Keeps the green gilet identity"],
    footer: "Same vest identity. Geometry is generated per archetype so each alpaca truly wears it.",
  } },
  "explorer-cape": { name: "Explorer Cape", draw: drawCape, hRef: "body", note: {
    adjustments: [
      "Cape flares from the neck to the chest per body",
      "Bottom width clamped to the silhouette (no stretch)",
      "Collar + clasp seated below the mouth line",
      "Drape folds scale with the flare",
      "Sleek widened/lifted; cria kept small + cute",
    ],
    system: ["Measures neck, shoulder, chest W, torso profile", "Generates a fitted variant PNG per archetype", "Prevents clipping into wool / covering the face", "Keeps the purple travel-cape identity"],
    footer: "Same cape identity. Geometry is generated per archetype so each alpaca truly wears it.",
  } },
