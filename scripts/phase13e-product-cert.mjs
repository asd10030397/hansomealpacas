 ...
 * HANSOME/BEER/GME/OKC × Cold5 × Warm5 × Force5
 * Global: no sticky clears, orphan runtime, zombie lease, dup publish, gen regression
...
const COLD_N = Number(process.env.HANSOME_CERT_COLD_N || 5);
const WARM_N = Number(process.env.HANSOME_CERT_WARM_N || 5);
const FORCE_N = Number(process.env.HANSOME_CERT_FORCE_N || 5);
...
async function waitTerminal(label, addr, { maxMs = 540_000 } = {}) {
  ...
  while (Date.now() - t0 < maxMs) {
    ...
    // Product gate: Locked BEER can be accepted even mid-deep if durable.
    if (
      f.lockState === "LOCKED_VERIFIED_ONCHAIN" &&
      f.positionsCount > 0 &&
      !f.cleared
    ...
    if (isTerminal(f)) return { ok: true, ...last, samples };
    await sleep(12_000);
  }
...
async function runToken(name) {
  ...
  for (let i = 1; i <= COLD_N; i++) {
    ...
    console.log(
      `${name} cold#${i} pass=${gate.pass} reason=${waited.reason} failed=${(gate.failed || []).join(",") || "-"} owner=${waited.f?.lockedOwner || "-"}`,
    );
  ...
  for (let i = 1; i <= WARM_N; i++) {
    ...
    if (waited.reason === "zombie_lease") out.globals.zombies += 1;
    if (waited.f?.lpGeneration) out.generations.push(waited.f.lpGeneration);
    console.log(`${name} warm#${i} pass=${gate.pass}`);
  }
  ...
  for (let i = 1; i <= FORCE_N; i++) {
    ...
    if (waited.f?.lpGeneration) out.generations.push(waited.f.lpGeneration);
    console.log(
      `${name} force#${i} pass=${gate.pass} lock=${waited.f?.lockState} tid=${waited.f?.lockedTokenId} recovery=${waited.f?.recoveryReason}`,
    );
  }

  // Generation regression: later generations should not silently rewind to empty when prior had positions
...
async function main() {
  ...
  const outPath = join(OUT_DIR, "phase13e_product_cert.json");
