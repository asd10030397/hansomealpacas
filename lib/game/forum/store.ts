...
/** MVP persistence — JSON file. TODO: migrate to Postgres/Redis for multi-instance production. */

let writeChain: Promise<void> = Promise.resolve();

function emptyStore(): ForumStoreData {
  return {
    version: 1,
    threads: [],
    replies: [],
...
function normalizeStore(parsed: ForumStoreData): ForumStoreData {
  return {
    version: 1,
...
function pruneNonces(nonces: ForumAuthNonce[]): ForumAuthNonce[] {
  const now = Date.now();
  return nonces.filter((n) => n.expiresAt > now);
}

function visibleThreads(threads: ForumThread[]): ForumThread[] {
...
export async function listForumThreads(
  ...
  return withStore((data) => {
    ...
    return visibleThreads(data.threads)
      ...
      .map(({ id, boardSlug: slug, title, authorAddress, tokenId, createdAt, replyCount }) => ({
        ...
        createdAt,
        replyCount,
        likeCount: countLikes(likes, "thread", id),
        viewerLiked: viewerHasLiked(likes, "thread", id, viewerAddress),
      }));
  });
}
...
export async function getForumThread(
  ...
  return withStore((data) => {
    const thread = visibleThreads(data.threads).find((t) => t.id === threadId);
    ...
    const replies = data.replies
      ...
      .map(({ id, threadId: tid, body, authorAddress, tokenId, createdAt }) => ({
        ...
        tokenId,
        createdAt,
        likeCount: countLikes(likes, "reply", id),
        viewerLiked: viewerHasLiked(likes, "reply", id, viewerAddress),
      }));
    return {
      thread: {
        ...thread,
        likeCount: countLikes(likes, "thread", thread.id),
        viewerLiked: viewerHasLiked(likes, "thread", thread.id, viewerAddress),
      },
      replies,
    };
  });
...
export async function issueForumNonce(address: string): Promise<{
  ...
  await mutateStore((data) => {
    data.nonces = pruneNonces(data.nonces).filter((n) => n.address !== normalized);
...
export async function consumeForumNonce(
  ...
  await mutateStore((data) => {
    data.nonces = pruneNonces(data.nonces);
...
export async function toggleForumLike(input: {
  ...
  await mutateStore((data) => {
    ...
    likeCount = countLikes(data.likes, input.targetType, input.targetId);
  });
