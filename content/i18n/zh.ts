...
export const zh: Messages = {
  ...
  litepaper: {
    meta: {
      ...
      description:
        "HANSOME ALPACAS 白皮書：創辦人的話、代幣經濟學、金庫政策、流動性政策、營收策略、路線圖與常見問題——只寫今天真正做到的事。",
    },
    nav: {
      onThisPage: "本頁目錄",
      sections: {
    ...
    hero: {
      ...
      subtitle: "一份公開紀錄，說明這個專案是什麼、不是什麼，以及打算如何成長——不做做不到的承諾。",
      meta: [
    ...
    founderLetter: {
      heading: "創辦人的話",
      paragraphs: [
        "我們並不是因為做出了什麼產品才開始 HANSOME ALPACAS。純粹是「一隻贏了基因樂透、卻什麼用都沒有的羊駝」這個想法讓我們自己笑了出來，我們猜也會讓別人笑出來。到現在，這仍然是整件事的全部前提，我們不打算把它包裝成別的樣子。",
        "我們能承諾的東西，比大部分專案上線時講的要少、也樸實得多：一個永遠不能再增發的固定供應量、一份任何人（包括我們自己）都無法更改的合約，以及每一個碰過這顆代幣的錢包的公開紀錄。這不是因為我們特別有原則，而是因為只有先拿掉「我們日後毀約」的能力，這些承諾才真正算數。",
        "這份文件是我們目前最接近「計畫」的東西。它說明今天什麼是真的、我們下一步打算做什麼、以及哪些仍然只是一個方向而已。遇到我們還沒有答案的地方，我們選擇老實說不知道，而不是硬掰一個聽起來有信心的答案。如果一年後 HANSOME 還在，我們希望原因是有一群真正的社群覺得值得留下來，而不是因為我們在最初把話說得太滿。",
    ...
    introduction: {
      heading: "簡介",
      paragraphs: [
        "HANSOME ALPACAS 是在 Robinhood Chain 上的迷因幣，透過 Uniswap v4 集中流動性池與 ETH 交易。它沒有產品可以賣給你，沒有功能可以承諾，也沒有把路線圖包裝成商業計畫。",
        "它真正擁有的，是固定且無法增發的供應量、一份沒有任何管理者特權的不可變更合約，以及每一個碰過這顆代幣的錢包的公開紀錄。這份文件的目的，就是老實說清楚今天什麼是事實、什麼只是期望，以及兩者之間的界線在哪裡——讓這個專案的信任建立在紀錄上，而不是說法上。",
      ],
      whatIsHansome: {
        heading: "HANSOME 是什麼？",
        paragraphs: [
          "HANSOME ALPACAS（代號：HANSOME）是一顆部署在 Robinhood Chain 上、供應量固定、0% 交易稅的 ERC-20 代幣。它唯一自稱的身分，就是它誕生的那個笑話：一隻極度帥氣、卻在設計上毫無用途的羊駝。",
          "代幣合約裡沒有質押機制、沒有遊戲、沒有收益機制，也沒有任何寫進合約裡的未來功能承諾。如果日後有任何改變，那會是另外一個清楚標示的新增項目——絕不會被事後補寫進這份文件，裝作那一直是原本的計畫。",
        ],
      },
    },
    vision: {
      heading: "願景",
      paragraphs: [
        "大部分迷因幣的設計就是衝一波然後淡去。這個專案想做的，是在第一波熱度過去之後，還能以一個值得留意的樣子存在——就算這代表成長得比較慢、也比較少噪音。",
        "目標不是特定的價格、市值或上架某個交易所。目標是圍繞一個真心覺得好笑的吉祥物，建立一個小而真實的社群；一個被保守、透明管理的金庫；以及一套不需要任何人「相信承諾」、只需要「看憑證」的流動性政策。",
      ],
    },
    philosophy: {
      heading: "核心理念",
    ...
    tokenomics: {
      ...
      distribution: {
        heading: "分配比例",
        body: "這個專案三個持有代幣的錢包，目前設定的分配目標如下：",
        rows: [
          { label: "流動性", value: "50,000,000 HANSOME（5%）", note: "已存入官方 Uniswap v4 流動性池。" },
          {
            label: "金庫",
            value: "900,000,000 HANSOME（90%）",
            note: "用於支援流動性、生態發展與未來計畫——詳見「金庫政策」。",
          },
          { label: "創辦人", value: "50,000,000 HANSOME（5%）", note: "創辦人配額。" },
        ],
        footnote:
          "以上是目前設定的分配目標。每個錢包即時的鏈上餘額都公開在 /transparency 頁面，且會隨著金庫依照下方政策把代幣投入流動性而隨時間變動。",
      },
      whyFixedSupply: {
        heading: "為什麼要固定供應量？",
    ...
    treasury: {
      heading: "金庫政策",
      intro: "金庫錢包目前持有 HANSOME 單一最大的配額。它是一個公開可查的單一錢包——目前還沒有拆分成幾個各自訂好固定比例的子帳戶。",
      lines: [
        {
          label: "流動性",
          value: "目前主要用途",
          detail: "到目前為止，金庫的支出大部分都用於挹注與加強官方的 Uniswap v4 流動性部位。",
        },
        {
          label: "社群",
          value: "規劃中——待定",
      ...
      transparencyBody:
        "上面提到的每一個錢包都是公開的。地址、目前餘額，以及 Uniswap v4 資金池／部位的細節，都即時更新在 /transparency 頁面——那個頁面，而不是這份文件，才是目前數字的第一來源。",
      wallets: [
        { title: "部署錢包", purpose: "合約部署與技術操作。", allocation: "0%" },
        { title: "流動性錢包", purpose: "官方 Uniswap v4 流動性管理。", allocation: "鎖定 LP——50,000,000 HANSOME + 0.075 ETH" },
        { title: "金庫", purpose: "金庫、生態發展、合作夥伴關係、未來流動性、行銷與開發。", allocation: "900,000,000 HANSOME（90%）" },
        { title: "創辦人錢包", purpose: "創辦人配額。", allocation: "50,000,000 HANSOME（5%）" },
      ],
    ...
    revenue: {
      heading: "營收策略",
      intro:
        "HANSOME 在設計上就是 0% 交易稅，所以不會像早期那些「抽稅型」迷因幣一樣自動產生收入。未來要挹注流動性或專案發展的資金，得從別的地方來。以下是目前已經在運作、或正在評估中的來源——這裡沒有任何一項是保證會實現的。",
      streams: [
        {
          id: "lp-fees",
          title: "LP 手續費",
          statusKey: "active",
          status: "已上線",
          body: "0.05% 的 Uniswap v4 資金池手續費，是目前唯一已經在運作的收入來源，會隨交易量自動增減。",
        },
        {
          id: "merchandise",
          title: "周邊商品",
          ...
          status: "探索中",
          body: "羊駝主題的周邊商品跟這個品牌很搭，但目前還沒有任何一款真正製作或販售。若之後真的推動，會再另外說明。",
        },
        {
          id: "partnerships",
          title: "合作夥伴",
          statusKey: "exploratory",
          status: "探索中",
          body: "目前沒有任何贊助、合作或聯名關係存在。待定——未來若真的簽署了任何合作，會在確定之後才對外公布。",
        },
        {
          ...
          statusKey: "exploratory",
          status: "探索中",
          body: "目前沒有規劃或開發中的額外產品（遊戲、工具、質押機制等）。我們刻意把這一類留白，而不是用一堆空想的功能點子把它填滿。",
        },
      ],
    },
    roadmap: {
    ...
    community: {
      ...
      paragraphs: [
        ...
        "這個專案寧願有一百個真心覺得吉祥物好笑、並且願意留下來的人，也不想要一萬個下週就忘記代號的人。這份文件裡的每一項政策——金庫的自律、流動性的做法、拒絕承諾不存在的功能——說到底都是為了同一個目標：給一個真實的社群，一個值得參與的理由。",
      ],
    },
    longTermVision: {
      heading: "長期願景",
      intro: "沒有人能保證這一定會成功。但我們期望 HANSOME 走向自給自足的樣子，是一個循環，而不是一條直線——每一個階段都餵養下一個階段：",
      lifecycle: [
        { label: "社群", body: "真正參與、而不只是旁觀的人。" },
        { label: "品牌", body: "一個讓人願意分享的吉祥物與識別。" },
        { label: "產品", body: "品牌能支撐的真實產出——周邊商品、聯名合作、工具（見「營收策略」）。" },
        { label: "營收", body: "來自這些產品與 LP 手續費的實際收入——不是空想。" },
        { label: "金庫", body: "這些收入以透明、公開、可上鏈查證的方式累積起來。" },
        { label: "流動性", body: "金庫資金依照既定節奏，強化並分散流動性。" },
      ],
      loopLabel: "循環",
      closing:
        "今天的 HANSOME，還處在這個循環最初的階段——主要是「社群」與「品牌」，「產品」與「營收」幾乎還是空白。這份文件會隨著情況改變被誠實地更新，而不會被改寫成好像一直都走在更前面的樣子。",
    },
    faq: {
      heading: "常見問題",
      items: [
