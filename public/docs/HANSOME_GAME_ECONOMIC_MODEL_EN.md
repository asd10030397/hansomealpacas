...
- \(G\) = GameTreasury **spendable** balance of `$HANSOME`  
  \(\displaystyle G = \mathrm{balance} - \mathrm{outstandingClaims}\)
...
| \(G_{\mathrm{safe}}\) | \(15{,}000{,}000\) HANSOME | Safe-mode threshold |
...
| \(G < G_{\mathrm{safe}}\) | \(0\) (+ Commit paused in implementation) |
...
3. **Incentive preservation** — while \(G \ge G_{\mathrm{safe}}\), a positive pool remains (until safe mode).  
...
\[
R_d^{A} = \Big\lfloor R_d \times 0.80 \Big\rfloor,\quad
R_d^{C} = \Big\lfloor R_d \times 0.10 \Big\rfloor,\quad
R_d^{H} = \Big\lfloor R_d \times 0.10 \Big\rfloor
\]
...
\mathrm{Reward}_i
\approx
\underbrace{\frac{R_{\mathrm{pool}} \cdot W_i}{\sum_j W_j}}_{\text{pool share}}
\cdot
...
**Example A — two Alpacas, no Cougars, both at Grassland (\(w=3\)), Common class, \(\pi=0\):**

\[
\Omega = 3+3 = 6,\quad
r_1 = r_2 = 320{,}000 \times \tfrac{3}{6} = 160{,}000
\]
...
- Alpaca receives gross from \(R_d^{A}\), then net after \(\pi > 0\).  
...
| Income drivers | Location weight, Farmer +20% weight, trait mitigation of \(\pi\) | Equal base split; hunt share if co-located with Alpacas |
| Primary risk | Hunting penalty \(\pi\) at non-Home locations | Failed hunts (\(R_d^{H}\) dust); competition among Cougars |
| Home | Allowed (safe from hunt \(\pi^0=0\)) | Illegal (invalid day) |
...
=
\sum_{s \in S}
\mathrm{Pr}(s)\cdot \mathrm{Reward}(s)
-
\mathrm{Cost}
\]
...
\[
R_d = \underbrace{\mathrm{playerTotal}}_{\text{reserved for claims}}
+ \underbrace{P_d^{\mathrm{pen}}}_{\text{stays in Treasury}}
+ \underbrace{\mathrm{unallocated}}_{\text{dust + empty pools + failed hunt } R_d^{H}}
\]
...
\[
R_d = f(G),\quad
f\text{ is a decreasing step function of }G\text{ relative to }G_0.
\]

- High \(G\) → larger \(R_d\) (up to \(R_0 = 400{,}000\)).  
- Falling \(G\) → automatic step-downs (\(280{,}000\) → \(160{,}000\) → \(80{,}000\)).  
- \(G < G_{\mathrm{safe}}\) → \(R_d = 0\) and Commits paused in the implementation.
...
| Ecosystem / Treasury only | No reason to play; NFTs idle | Positive \(R_d\) while \(G \ge G_{\mathrm{safe}}\); asymmetric pools; pull claims |
...
| Gross runway at \(G_0/R_0 \approx 750\) days is an **illustration** | That illustration equals real calendar life |
...
| Item | Value |
|------|------|
| \(G\) | \(15{,}000{,}000 \le G < 60{,}000{,}000\) → \(R_d=80{,}000\); \(G < 15{,}000{,}000\) → \(R_d=0\) |
