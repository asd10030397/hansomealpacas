...
| \(G_{\mathrm{safe}}\) | \(15{,}000{,}000\) HANSOME | 安全模式門檻 |
...
| \(G < G_{\mathrm{safe}}\) | \(0\)（實作另暫停 Commit） |
...
3. **維持誘因** — 在 \(G \ge G_{\mathrm{safe}}\) 時仍保留正獎池（進入安全模式前）。  
...
\mathrm{Reward}_i
\approx
\underbrace{\frac{R_{\mathrm{pool}} \cdot W_i}{\sum_j W_j}}_{\text{獎池份額}}
\cdot
\underbrace{(1-\pi_i)}_{\text{生存／狩獵風險}}
\cdot
\underbrace{\tau_i}_{\text{特質效果}}
\]

**羊駝（鏈上精確）：**

1. 地點權重 \(w(L) \in \{1,2,3,5,8\}\)（Home…River）。  
2. 有效權重：Farmer 為 \(1.20 \times w(L)\)，否則 \(w(L)\)。  
3. 毛額：\(r_i^{A,\mathrm{gross}} = R_d^{A} \cdot \omega_i / \Omega_d\)（最後一位吸收餘數）。  
4. 懲罰率 \(\pi_i\) 由地點壓力與特質決定。  
5. 淨額：\(r_i^{A,\mathrm{net}} = r_i^{A,\mathrm{gross}}(1-\pi_i)\)。
...
**例 A — 兩隻羊駝、無美洲獅，皆在 Grassland（\(w=3\)）、Common、\(\pi=0\)：**
...
羊駝先得 \(R_d^{A}\) 毛額再扣 \(\pi\)；美洲獅得基礎池（獨佔時全額）與成功狩獵時的狩獵份額。羊駝懲罰回流國庫，**不**付給美洲獅錢包。
...
| 收益驅動 | 地點權重、Farmer +20% 權重、特質降低 \(\pi\) | 均分基礎池；與羊駝同地才有狩獵份額 |
| 主要風險 | 非 Home 地點狩獵懲罰 \(\pi\) | 狩獵失敗（\(R_d^{H}\) 粉塵）；美洲獅彼此競爭 |
| Home | 允許（\(\pi^0=0\)） | 非法（當日無效） |
...
**重要：** \(\mathrm{EV}\) 可為負。高技術不代表扣除 NFT 成本後仍獲利。
...
\[
R_d = \mathrm{playerTotal} + P_d^{\mathrm{pen}} + \mathrm{unallocated}
\]
...
營運上傾向將 GameTreasury 靠近 \(G_0 = 300{,}000{,}000\)，使早期落在 \(R_d = R_0\) 檔，實際以入金與後續動態為準。
...
健康的 GameFi 需要**雙方一起成長**：無限抽取會摧毀遊戲；滿國庫無玩家則無文化。HANSOME 透過 \(G \mapsto R_d\) 與非對稱角色耦合雙方。
...
\[
R_d = f(G),\quad
f\text{ 為相對於 }G_0\text{ 的遞減階梯函數。}
\]

- \(G\) 高 → \(R_d\) 較大（最高 \(R_0 = 400{,}000\)）。  
- \(G\) 下降 → 自動降檔（\(280{,}000\) → \(160{,}000\) → \(80{,}000\)）。  
- \(G < G_{\mathrm{safe}}\) → \(R_d = 0\)，實作暫停 Commit。
...
| 僅生態／國庫 | 無人願玩；NFT 閒置 | \(G \ge G_{\mathrm{safe}}\) 時仍有正 \(R_d\)；非對稱池；拉式 Claim |
...
| 固定供應阻擋獎勵通膨 | 保證代幣價格 |
| 雙邊玩法支持留存設計 | 保證玩家獲利 |
| \(G_0/R_0 \approx 750\) 天粗略跑道為**示意** | 該示意等於真實日曆壽命 |
...
| 項目 | 數值 |
|------|------|
| \(G\) | \(15M \le G < 60M\) → \(R_d=80{,}000\)；\(G < 15M\) → \(R_d=0\) |
